...
simple arithmetic problems in base 8 for a student to answer*/
...
/*sets constants for how many questions are asked, 
 *how many retries are allowed and the range for correct answers 
 *to ensure no addition problem answer is above 24 and no subtraction one is below 0
 */
	
private static final int NUM_PROBLEMS = 5;
private static final int MAX_SUM = 24;
...
//sets variables for numbers in question depending on the operation type
	...
	correctAnswer = convertToBase8(correctAnswer);
	
//sets counter for tries after a wrong answer and sets indefinite loop until
//the counter is reached
	int tryCounter = TRIES;
	while (tryCounter > 0) {
	int userAnswer = readInt("What is " + firstNumber + (operation.equals("+") ? " + " : " - "  ) + secondNumber + " in base 8? ");
...
/*
 * Converts decimal correct answer to base 8
 */
private int convertToBase8 (int number) {
	String convertedNumber = "";
	convertedNumber = Integer.toString(number, 8);
	return Integer.parseInt(convertedNumber);
}

//generates a random kudos response upon input of corret answer
